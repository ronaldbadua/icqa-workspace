  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-sm font-medium text-sky-700 hover:bg-sky-100 disabled:opacity-60">
      ...
      {!search.trim() && (
        <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-6 py-10 text-center">
          ...
          <p className="text-sm text-slate-500">
            Type a keyword above to search through{" "}
            <span className="font-medium text-slate-700">{entries.length}</span> record{entries.length !== 1 ? "s" : ""}.
          </p>
          <p className="mt-1 text-xs text-slate-400">
            Import a Word document to add records, or use the REST API.
          </p>
        </div>
      )}
    </div>
